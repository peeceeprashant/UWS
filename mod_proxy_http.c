...
{
     ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r,"Custom mod_proxy_http "); 
    int status;
    ...
    ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r,"Backend : %s", backend->hostname);
    ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r,"Backend : %s", proxyname); 
    ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r,"Backend : %s", proxyport); 
    ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r,"Backend : %s", server_portstr);
    ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r,"Backend : %s", uri);
    ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r,"Backend : %s", url); 

    //Failsafe Code Starts Here
    if(!strcmp(url, "/failsafe")) {
      ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r,"Failsafe activated");
      char* timegateUrl = "/sitestory/timegate/http://";
      char* previousHostname = r->prev->hostname;
      char* previousURI = r->prev->uri;

      char* finalURI = apr_pstrcat(r->pool, timegateUrl, previousHostname, previousURI, NULL);
      ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r,"Sitestory Timegate Request URI: %s", finalURI);
   
      //Set URI to failsafe URI
      url = finalURI;
    }
    //Failsafe Code Ends Here
    ...
    //ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r,"Previous URI: %s", r->prev->unparsed_uri);
    ...
    ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r,"Hostname : %s", backend->hostname);
    ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r,"Backend : %s", proxyname);
    ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r,"Backend : %s", proxyport);
    ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r,"Backend : %s", server_portstr);
    ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r,"Backend : %s", uri);
